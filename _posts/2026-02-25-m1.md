...
image:
  path: assets/img/posts/causal-landscape.jpg
  class: "img-right"
categories: [AI]
tags: [AI]  
---
...
# Causal direction from convergence time: how training speed tells you which variable causes which
...
> *[image suggestion: the ladder of causation as a literal three-rung ladder. rung 1 at the bottom: "seeing — $P(Y \mid X=x)$" with icons representing "regression, neural nets, GPT-4, all of ML." rung 2 middle: "doing — $P(Y \mid \mathrm{do}(X=x))$" with icons representing "RCTs, interventions, policy actions." rung 3 top: "imagining — $P(Y_{X=x'} \mid X=x, Y=y)$" with icons representing "counterfactuals, legal reasoning, regret." a large red wall labeled "Pearl's 1995 impossibility theorem" between rungs 1 and 2. a smaller wall labeled "requires abduction + twin networks" between rungs 2 and 3. GPT-4's logo stuck at the bottom rung with a cartoon wall blocking it from climbing.]*
...
> *[image suggestion: plot of $Y = X^2$ with $X$ ranging from $-2$ to $2$. draw two arrows converging to the same point $Y = 1$: one from $X = 1$ and one from $X = -1$. label this with a big question mark: "which $X$ produced this $Y$?" then show the conditional distribution $P(X \mid Y=1)$: it has two equal probability peaks at $x=1$ and $x=-1$. so $E[X \mid Y=1] = 0$. the optimal reverse predictor is identically zero regardless of $Y$. make this visual.]*
...
> *[image suggestion: four-panel plot. top row for $Y = \sin(X) + \varepsilon$, forward direction. left panel: scatter plot of (X, forward residual) at step 50 — visible structured pattern, points clearly not randomly scattered. right panel: same scatter at step 500 — pure noise, no pattern. bottom row, reverse direction same DGP. left panel: scatter of (Y, reverse residual) at step 50 — structured. right panel: same at step 3000 — still structured, correlation visible, hasn't disappeared. the persistence in the bottom row is Lemma 1 made visual.]*
...
> *[image suggestion: 3D surface plots of MSE loss as a function of two network parameters. left: forward direction loss for $Y = \sin(X) + \varepsilon$. smooth bowl shape, single clear minimum, surface looks like a nice concave valley. right: reverse direction loss for same DGP. rough terrain, visually higher floor, the surface has bumps and ridges reflecting the position-dependent conditional variance $\text{Var}(X \mid Y=y)$. color the surface by height. the contrast between smooth bowl and rough terrain is the whole point of Lemma 2.]*
...
under PL with constant $\mu$, learning rate $\eta$, and gradient noise variance $\sigma^2$, the standard SGD convergence bound gives:
...
> *[image suggestion: the exact canonical loss curves. x-axis is training step (0 to 3000). y-axis is held-out MSE on log scale (from $10^{-3}$ to $1$). forward (solid blue): starts high near 1, smooth decrease, crosses the dashed horizontal line at $\tau = 0.05$ at step 161 (marked with a dot), continues falling toward the noise floor. reverse (dashed red): starts similarly high, decreases for first few hundred steps, plateaus just above the $\tau = 0.05$ line, stays flat until hitting step 3000 cap. label the CCA score $= -2839$ in the corner. this is the single most important figure in the paper.]*
...
> *[image suggestion: two panels. left: bar chart of all 108 CCA scores sorted by absolute magnitude, descending from left to right. blue bars are correct predictions, red bars are wrong. nearly all bars at the far left (high confidence) are blue. red bars concentrate in the middle-right region (low confidence, scores near zero). right: a cumulative accuracy plot. x-axis is threshold on $|\text{CCA}|$, y-axis is accuracy among pairs with $|\text{CCA}|$ above threshold. starts near 100% for the most confident predictions and stays well above the 72.2% majority-class line and the 63% RESIT line throughout.]*
...
> *[image suggestion: clean flowchart of the four stages. stage 0: box labeled "observational data" → "PC-stable" → "undirected skeleton $G_0^\text{skel}$." stage 1: "observational data" + "$G_0^\text{skel}$" → "variational CIB encoder" → "representation $T_0$." stage 2: "interventional data" + "$T_0$" → "XGES with MDL + CCA scoring" (this box should have a sub-note: "CCA scoring requires training two MLPs per candidate edge") → "oriented graph $G_1$." stage 3: "interventional trajectories" + "$G_1$" + "$T_0$" → "Bareinboim CRL policy optimization" → "policy $\pi_1$." then a big arrow from stages 1-3 forming a cycle labeled "alternating loop until convergence." annotations: "stages 0-1: observational data only. stages 2-3: require interventional data."]*

---

## honest accounting of where this breaks
...
## prerequisites: everything you need to actually understand the math in this blog
...
### what independence actually means, with the math
...
> *[image suggestion: a 2D loss landscape (a smooth bowl) with a red dot starting at the rim. show several steps of gradient descent: arrows pointing downhill, each successive dot moving toward the minimum. annotate: "each arrow = one gradient step. arrow length = learning rate × gradient magnitude." show a second scenario where the learning rate is too large: the red dot oscillates back and forth past the minimum without settling. this makes the learning rate tradeoff visual.]*
...
> *[image suggestion: a carefully labeled loss curve from training. x-axis: training steps 0 to 3000. y-axis: MSE loss on log scale. the curve starts around 1.0, drops steeply in the first few hundred steps, then curves and approaches the noise floor. annotate: "initialization" at step 0, "fast descent (large gradients)" in the first portion, "slow descent (gradients shrinking)" in the middle, "noise floor $= \sigma_\varepsilon^2$" as a horizontal dashed line near the bottom, "convergence threshold $\tau = 0.05$" as another dashed horizontal line. show the forward network crossing $\tau$ and the reverse network plateauing above it.]*
...
> *[image suggestion: three DAGs side by side. left: chain $X \to Z \to Y$ with annotation "$X \perp Y \mid Z$ (mediator)". middle: fork $X \leftarrow Z \rightarrow Y$ with annotation "$X \perp Y \mid Z$ (confounder)". right: collider $X \rightarrow Z \leftarrow Y$ with annotation "$X \perp Y$ but $X \not\perp Y \mid Z$ (collider!)". these are the three fundamental graph motifs and knowing them is enough to understand most of causal discovery.]*
...
> *[image suggestion: a Venn diagram of entropy. left circle labeled $H(X)$. right circle labeled $H(Y)$. the overlap between the two circles is $I(X;Y)$ — the shared information. the left-only part is $H(X \mid Y)$ — information in $X$ not explained by $Y$. the right-only part is $H(Y \mid X)$. the union is $H(X, Y)$ — total joint entropy. annotate each region clearly. this is the standard entropy Venn diagram and it makes mutual information immediately intuitive.]*
...
*end of prerequisites. all mathematical tools used in the main blog are defined above. if anything in the main text still feels unclear, the definitions and derivations above should fill the gaps.*
