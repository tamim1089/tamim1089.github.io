...
tanh is one common activation:

$\sigma(z) = \frac{e^{z} - e^{-z}}{e^{z} + e^{-z}}$.

It outputs values between $-1$ and $1$, is smooth (differentiable everywhere), and saturates for large $|z|$.

ReLU is another activation:

$\sigma(z) = \max(0, z)$.

It outputs zero for negative inputs and acts as the identity for positive inputs. It is computationally cheap, does not saturate for positive inputs, and is widely used in practice.
...
$K(x)$ is uncomputable: there is no algorithm that can take any string $x$ and return its Kolmogorov complexity. This is related to the halting problem. However, it serves as a theoretical foundation for information theory and the idea that "simpler explanations are better."

Minimum Description Length (MDL), introduced by Rissanen in 1978, is the practical approximation. Prefer the model $M$ that minimizes

$\mathrm{MDL}(M) = \lvert M \rvert + \lvert D \mid M \rvert$:

the number of bits needed to describe the model, plus the number of bits needed to describe the data given the model. A more complex model requires more bits to describe itself but may compress the data better. Simpler models use fewer bits to describe themselves but may not compress the data as well. The optimal MDL balances both.
...
You could measure prediction error as $\lvert g_{\theta}(X) - Y \rvert$ (absolute value, also called L1 loss) instead of $\bigl(g_{\theta}(X) - Y\bigr)^{2}$ (MSE, L2 loss).
...
The CCA framework requires three conditions on this ANM. First, $f$ must be nonlinear. Second, $f$ must be Lipschitz continuous, meaning $\lvert f(x) - f(x') \rvert \le L_{f} \lvert x - x' \rvert$ for some constant $L_{f}$ — the function cannot change infinitely fast anywhere. Third and most importantly, $f$ must be injective (one-to-one), meaning $f(x_{1}) = f(x_{2}) \implies x_{1} = x_{2}$. Distinct inputs must always produce distinct outputs.
...
There is a counterintuitive empirical finding worth noting. If you measure instantaneous gradient norm variance at a single training step, the ratio (reverse / forward) for $Y = X^{3}$ (z-scored) is $0.376$ at initialization and $0.054$ at mid-training — both below $1$. This appears to suggest that the forward direction has larger gradient variance, which contradicts the narrative.

The explanation is that the forward network is learning the mapping $x \mapsto x^{3}$, a function with large Jacobian norms (since $\lVert f'(x) \rVert$ for a cubic is $\lvert 3x^{2} \rvert$, which can be large). The reverse network is fitting $\mathbb{E}[X \mid Y] \approx 0$ (near-constant for a symmetric input distribution), which has small Jacobian norms. Thus, instantaneous gradient variance is dominated by Jacobian magnitude rather than the residual correlation structure.

For $Y = \sin(X)$, the ratio is $1.161$ at initialization and grows to $2.179$ at mid-training. In this case, the reverse target $\mathbb{E}[X \mid Y]$ involves averaging over multiple branches of the inverse sine, producing a non-trivial function with larger Jacobians in the reverse network.

Which mechanism dominates depends on the specific data-generating process. The correct proxy for Lemma 2's landscape difficulty is convergence time, not instantaneous gradient statistics.
...
Lemma 3 states that two separate conditions apply in the reverse direction, and both require more optimization steps.

Condition (a): If the convergence threshold $\tau$ lies between the two minimum losses (i.e., $\mathcal{L}^{*}_{\text{fwd}} < \tau < \mathcal{L}^{*}_{\text{rev}}$), the forward network can reach $\tau$, but the reverse network cannot. Its noise floor lies above $\tau$, so infinite training does not help. In this case, $T_{\text{fwd}}$ is finite, while $T_{\text{rev}} = T_{\max}$ (it hits the iteration cap).

Condition (b): When $\tau$ is above both minima, the reverse network still requires more steps for two reasons. First, it must close a larger effective gap, since $\mathcal{L}_{0} - \tau$ is larger in the reverse direction when $\mathcal{L}^{*}_{\text{rev}} > \mathcal{L}^{*}_{\text{fwd}}$. Second, the non-separable gradient covariance inflates the effective $\sigma^{2}$ in the noise-floor term, increasing the bias and pushing the optimizer away from the threshold more persistently.
...
**MDL alone compresses spurious correlations.** Minimum Description Length follows from Kolmogorov complexity. Kolmogorov complexity $K(x)$ is the length of the shortest program on a universal Turing machine that outputs $x$. Solomonoff (1964) proved that the optimal prior over hypotheses weights each by $2^{-K(h)}$, so shorter descriptions receive exponentially more prior probability. Rissanen (1978) gave the computable approximation: prefer the model minimizing $\lvert M \rvert + \lvert D \mid M \rvert$ (the description length of the model plus the description length of the data given the model). The Algorithmic Markov Condition (Janzing and Schölkopf 2010) states that the true causal factorization $P(X)\,P(Y \mid X)$ has shorter Kolmogorov complexity than $P(Y)\,P(X \mid Y)$, because independent components compress independently.
...
Term 3 is MDL on graph $G$. It is approximated via BIC: $\mathrm{MDL}(G) \approx -\ell(G) + \frac{\lvert E_{G} \rvert}{2}\log n$. This is the log-likelihood of the data under the graph minus a penalty for the number of edges, driving the solution toward sparser graphs. Kaltenpoth and Vreeken showed that for linear Gaussian SCMs, refined MDL is asymptotically equivalent to BIC, connecting this formulation to classical model selection theory.

Term 4 is CCA. It enters through the XGES scoring function: $\mathrm{Score}(G) = \mathrm{MDL}(G) + \lambda_{3} \cdot \mathrm{CCA}(G)$. CCA adds a bonus (negative score = forward faster = support for $X \to Y$) or penalty (positive score = reverse faster = evidence against $X \to Y$) on top of the MDL score for each candidate edge orientation. XGES then searches for the graph maximizing this combined score.

**Theorem 3: The MDL Regularization Threshold for Spurious Edge Exclusion.** At any local minimum of $\mathcal{L}_{\text{CCL}}$, if $\lambda_{2} \ge \frac{(1-\gamma)\log \lvert V \rvert}{\lvert E_{\max} \rvert}$, spurious edges are excluded asymptotically as $n \to \infty$. The proof is as follows: a spurious edge at finite $n$ can gain at most $O(\log n / n) \to 0$ in MDL likelihood (from the BIC approximation). Adding a spurious edge costs at least $(1-\gamma)\log \lvert V \rvert$ in MDL complexity. Above the threshold, the cost exceeds the benefit asymptotically. For the three-variable experiment with $\lvert V \rvert = 3$, $\lvert E_{\max} \rvert = 3$, and $\gamma = 0.9$, the threshold is approximately $\frac{0.1 \cdot \ln 3}{3} \approx 0.037$. At $\lambda_{2} = 0.5$ (well above threshold), there are zero spurious edges. At $\lambda_{2} = 0.1$ (still above threshold), one spurious edge persists because the non-injective mechanism $X_{2} = X_{1}^{2}$ creates a misleading CCA signal.
...
**Stage 0:** Run PC-stable (Colombo and Maathuis 2014) on observational data to recover the undirected skeleton — i.e., which pairs are connected, without direction information. PC-stable recovers the skeleton with probability $1 - \delta$ for $n \ge O\!\left(\frac{\log(\lvert V \rvert^{2}/\delta)}{\alpha^{2}}\right)$, where $\alpha$ is the minimum partial correlation magnitude.

**Stage 1:** Train the causal IB encoder $T_{0}$ via variational CIB conditioned on $G_{0}^{\text{skel}}$. Use a VAE architecture with diagonal covariance. The GEM (Generalized EM) property ensures monotone decrease of the CIB objective.

**Stage 2:** Run XGES with scoring function $\mathrm{Score}(G) = \mathrm{MDL}(G) + \lambda_{3} \cdot \mathrm{CCA}(G)$ over $T_{0}$ and interventional data. XGES (Nazaret and Blei 2024) is an extremely greedy equivalence search extending GES (Chickering 2002) to improve computational efficiency while maintaining correctness guarantees. The CCA scoring for each candidate edge requires training two MLPs and measuring convergence time.

**Stage 3:** Optimize $\pi_{1}$ via Bareinboim CRL (2024) over $G_{1}$ and $T_{0}$. Perform an identifiability check before each policy gradient step.

**Alternating Loop:** Update $T$ by minimizing $I(X;T) - \beta I_{c}(Y \mid \mathrm{do}(T))$, update $G$ using XGES with MDL + CCA, update $\pi$ via policy gradient subject to identifiability constraints, and repeat until $\lvert \Delta \mathcal{L}_{\text{CCL+}} \rvert \le \varepsilon_{\text{conv}}$.

**Output:** Causal graph $G$, compressed representation $T$, causal policy $\pi$, sample complexity certificate $n^{*} = C \cdot \hat{\tau}_{\text{mix}} \cdot d_{c}(G) \cdot \frac{\log(d_{c}(G)/\delta)}{(1-\gamma)^{3}\varepsilon^{2}}$.
...
All seven runs show strictly monotone decrease. Theorem CCL+ is confirmed: the objective goes down at every iteration without exception.

The spurious edge story is equally clean. One spurious edge persists at $\lambda_2 \le 0.2$, and it disappears at $\lambda_2 = 0.5$. Why exactly $0.5$? The theoretical threshold for this SCM is

$\lambda_2 \ge \frac{(1-\gamma)\log \lvert V \rvert}{\lvert E_{\max} \rvert} \approx 0.037$.

The value $0.5$ is well above that threshold. The reason spurious edges survive at lower values in practice, despite being above the asymptotic threshold, is the non-injective mechanism $X_2 = X_1^{2}$. Recall from the boundary conditions section: the reverse network on that edge collapses to predicting zero in under $25$ steps, producing a misleading CCA score that partially offsets the MDL penalty.

The spurious edge survives because two errors are compounding. The monotone convergence holds regardless. The theorem guarantees that the objective decreases, not that the graph is correct.
