...
tanh: $\sigma(z) = \frac{e^z - e^{-z}}{e^z + e^{-z}}$. this squashes any real number into the range $(-1, 1)$. it\'s smooth and differentiable everywhere, which is useful for gradient-based optimization. it saturates for large $|z|$, meaning the output gets stuck near $-1$ or $+1$ and the gradient becomes very small, which can slow training.
...
you could define prediction error as $|g_\theta(X) - Y|$ (the absolute value, L1 loss) instead of $(g_\theta(X) - Y)^2$ (MSE, L2 loss). both are valid loss functions. they produce different optimal predictors and have different mathematical properties.
...
geometrically: the slope of $f$ never exceeds $L$ in magnitude anywhere. $\sin(x)$ is globally Lipschitz with constant 1 because $|\cos(x)| \leq 1$ everywhere. a function like $x^3$ is not globally Lipschitz because $|(x^3)\' | = 3x^2$ grows without bound as $x \to \infty$, but it is Lipschitz on any bounded interval.
...
the Jacobian generalizes the gradient to vector-valued functions: for $g: \mathbb{R}^n \to \mathbb{R}^m$, the Jacobian $J$ is the $m \times n$ matrix $J_{ij} = \partial g_i / \partial x_j$. for a scalar-output neural network $g_\theta: \mathbb{R} \to \mathbb{R}$, the Jacobian with respect to the parameters $\theta$ is just $\nabla_\theta g_\theta$. the Jacobian norm $\|J\|$ measures how much a small change in inputs stretches the outputs. the gradient variance analysis in Lemma 2 depends critically on Jacobian norms: for the cubic DGP $Y = X^3$, the Jacobian of the target function is $f\'(x) = 3x^2$, which can be large for large $|x|$, and this competes with the residual correlation effect in determining which network has larger gradient variance.
...
**MDL alone compresses spurious correlations.** Kolmogorov complexity $K(x)$ is the length of the shortest program outputting $x$ on a universal Turing machine. Solomonoff (1964) showed the optimal prior over hypotheses weights each by $2^{-K(h)}$: shorter descriptions are exponentially more probable. Rissanen (1978) gave the computable approximation: minimize $|M| + |D \mid M|$. the Algorithmic Markov Condition says the true causal factorization $P(X)P(Y \mid X)$ has shorter total Kolmogorov complexity than $P(Y)P(X \mid Y)$.
...
**MDL regularization threshold theorem.** at any local minimum of $\mathcal{L}_\mathrm{CCL}$, if $\lambda_2 \geq \frac{(1-\gamma)\log|V|}{|E_\mathrm{max}|}$, spurious edges are excluded asymptotically as $n \to \infty$. the argument: a spurious edge gains at most $O(\log n / n) \to 0$ in MDL likelihood at finite $n$. adding a spurious edge costs at least $(1-\gamma)\log|V|$ in MDL complexity. above the threshold, the complexity cost exceeds the likelihood gain asymptotically. for the three-variable experiment ($|V| = 3$, $|E_\mathrm{max}| = 3$, $\gamma = 0.9$), the threshold is approximately $\frac{0.1 \cdot \ln 3}{3} \approx 0.037$.
...
**alternating loop.** update $T$ by minimizing $I(X;T) - \beta I_c(Y \mid \mathrm{do}(T))$, update $G$ using XGES with MDL + CCA scoring, update $\pi$ via policy gradient subject to identifiability, repeat until $|\Delta\mathcal{L}_\mathrm{CCL+}| \leq \varepsilon_\mathrm{conv}$.
