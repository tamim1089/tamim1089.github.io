...
Before you roll your eyes and close this tab, let me be clear about what this actually is. This is not a claim that AI does not work. GPT-4 exists. AlphaFold cracked the protein folding problem that stumped biologists for fifty years. Machine translation works. Nobody serious disputes those achievements. This is about what we destroyed to get there.

Between 2012 and 2024, AI research went from exploring six or seven competing ideas to optimizing exactly one. Not because that one approach was proven to be the best, but because it happened to fit hardware we already owned. We built an entire scientific field around what was convenient, called it progress, and buried everything else. The researchers who built alternative frameworks for understanding intelligence, rigorous and mathematically solid frameworks with decades of history behind them, retired without training anyone to carry on their work. The grad students who might have learned those ideas never did, because universities stopped teaching them. The infrastructure needed to test those ideas does not exist anymore, because nobody ever funded it.
...
Sara Hooker at Google Brain came up with this term. A research idea wins the hardware lottery when it succeeds because it fits whatever hardware already exists, not because it is actually the best solution to the problem.

History is full of this. Charles Babbage designed the Analytical Engine in 1837 and the design was mathematically correct, but Victorian fabrication could not build it. Perceptrons worked conceptually when Rosenblatt published them in 1958, died when Minsky and Papert proved they could not solve XOR in 1969, then came back in the 1980s when hardware finally existed that could train multi-layer networks. Backpropagation was theoretically viable since Rumelhart, Hinton, and Williams published it in 1986, but nobody could scale it until GPUs arrived.
...
Glushkov spent the rest of his life dividing his time between Moscow and Kyiv, pushing truncated versions of the project through defence ministries that wanted the efficiency gains without the distributed network. He died in January 1982, still working, of a brain tumour. He was 58 years old. In the last nine days of his life, having regained consciousness from a coma, he dictated his memories and ideas to his daughter Olga.
...
But it provides the theoretical ideal against which all practical learning algorithms can be measured. Jorma Rissanen's Minimum Description Length from 1978 is a computable approximation: choose the hypothesis that minimises the combined length of the model description plus the data description given the model. This formalises Occam's Razor precisely. Good learning is compression. If a model genuinely understands a pattern, it compresses the data that instantiates that pattern. A child who learns addition does not store every arithmetic fact they have seen. They store a compact rule. That is what learning looks like mathematically.

Now look at GPT-4 by that standard. 1.8 trillion parameters trained on several trillion tokens of text. The model is larger than the subset of its training data you would need to describe almost everything it knows. That is not compression. That is memorization at scale with a different name.

Solomonoff's framework predicts exactly what happens when you push these systems slightly outside their training distribution. They fail. Because they did not compress a rule. They stored a weighted average of examples. The field has names for this, stochastic parrot for language models, shortcut learning for vision models, but nobody has connected these failures back to Solomonoff, who called all of this in 1964. Each generation of models is exponentially larger than the last. Capabilities scale sublinearly with size. You put in ten times the compute and get back thirty percent more performance. Solomonoff told you this road does not lead to general intelligence. What it leads to is very good interpolation inside a very large training distribution. Which is useful. Commercially valuable. And not what is being sold.

Nobody measures compression. Bits per parameter does not appear on a leaderboard.
...
Ali Rahimi, a Google researcher, stood up at NeurIPS 2017 to accept his Test of Time award and told the assembled audience that machine learning had become alchemy. he showed that researchers had stripped most of the complexity from a state-of-the-art translation algorithm and it translated better, meaning the original creators did not understand what they had built. he meant practitioners who add batch normalisation, dropout, or a particular learning rate schedule because it always works, with no understanding of why it works or when it will fail.

He offered a specific example that stays with you. a research team changed the rounding mode of floating point arithmetic in tensorflow. not the data, not the architecture, not the training procedure. only the direction in which decimal numbers were rounded when they exceeded floating point precision. the model's error rate changed from 25% to 99%.
...
The brittleness is not isolated to rounding modes. the adversarial examples literature, starting with Szegedy in 2013, shows you can take any image correctly classified by a state-of-the-art vision model and add pixel-level perturbations invisible to a human observer that cause the model to classify the image as anything the attacker chooses. the perturbations exploit the geometry of the model's decision surface, and they work because that surface is not grounded in the way a robust classifier would need to be. a human does not reclassify a panda as a gibbon because you added imperceptible static. the human has a concept. the model has a decision boundary that happens to agree with human labels on the training distribution.

Both failure modes come from the same place. fitting data without compressing it produces surfaces that are arbitrarily sensitive to irrelevant variations. the field has not fixed this after a decade of research because fixing it would require abandoning the hardware lottery architecture. robust adversarially stable classifiers exist and can be proved to exist, but training them costs orders of magnitude more than training standard networks, which means nvidia makes less money from them, which means the incentive to solve the problem correctly is structurally suppressed.
...
Real neurons do not do continuous mathematics. They spike. Discrete voltage spikes, that is the entire mechanism. The human brain achieves 86 billion neurons on 20 watts through event-driven computation where energy gets consumed only when neurons actually fire, sparse activation where only one to four percent of neurons are active at any moment, and temporal coding where information encodes in spike timing rather than activation magnitude.
...
The opportunity cost is concrete. If neuromorphic hardware had received GPU-level development investment from 2012 to 2024, equivalent model training runs at 100 times lower energy would mean 50,000 megawatt-hours becomes 500. We did not fund that path because nobody's quarterly revenue depended on it.
...
Between 70 and 95% of all ai chips sold globally come from Nvidia. this is not a natural monopoly in the classical sense. it is the product of CUDA, Nvidia's proprietary parallel computing platform introduced in 2006, which has accumulated 18 years of optimized libraries, debugging tools, profiling infrastructure, and compiler optimization that simply does not exist for any alternative hardware.
...
The mixture-of-experts architecture was first proposed in 1991. it achieves dramatically better computational efficiency by activating only a subset of model parameters for any given input. DeepSeek's demonstration in early 2025 that a mixture-of-experts model matched gpt-4's performance at roughly 6% of the training cost was treated as a surprise. it should not have been. the technique was 34 years old. the reason it had not been pursued aggressively by well-funded American labs is that mixture-of-experts reduces compute requirements, and reducing compute requirements reduces cloud revenue for Amazon, Microsoft, and Google, all of whom are simultaneously ai investors and compute sellers.
...
Stanford's course catalog is publicly archived. In 2008, Stanford offered CS 221 covering search, knowledge representation, logic, and planning, CS 228 on Probabilistic Graphical Models, CS 229 on Machine Learning including theoretical analysis, and CS 154 on Formal Languages and Automata. By 2024, CS 221 still exists but the curriculum shifted heavily toward learning methods. Courses focused on sample complexity theory, formal verification of AI systems, and cognitive modeling either vanished or became obscure electives with minimal enrollment. The expertise required to build hybrid neuro-symbolic systems, to reason about generalisation bounds, to design interpretable rule-based systems, is no longer produced by graduate training.
...
What he said specifically is worth understanding. Hinton stated publicly that he does not believe backpropagation is what the brain is doing. this matters because the entire theoretical framing of the neural network approach, the reason we call these systems neural networks rather than something more accurate like high-dimensional curve fitting machines, depends on the analogy to biological neural networks. that analogy gets used in peer-reviewed literature to argue that scaling these systems will eventually produce human-like intelligence. if the person who built them does not believe the core algorithm matches what the brain does, the analogical foundation of the enterprise collapses.

Biological learning mechanisms, predictive coding, Hebbian learning, spike-timing-dependent plasticity, share a property backpropagation lacks. they are local. each neuron updates based on its own activity and the activity of its immediate neighbors, without requiring a global error signal propagated backward through the entire network. that locality is why the brain runs on 20 watts. backpropagation requires computing the entire network's error globally and transmitting it backward through every layer simultaneously, which is why training gpt-4 required 50,000 megawatt hours of electricity.

Hinton also observed that current ai scaling works by running identical copies of the same model on different processors and synchronizing their weight updates. you cannot synchronize ten thousand human brains. biological intelligence does not work by running identical copies of itself on parallel substrates and averaging their gradients. each human brain is structurally unique, shaped by unique developmental history and unique embodied experience. when you run 10,000 identical copies of a transformer and synchronize their gradients, you are doing something with no biological analog, and Hinton explicitly identifies it as architecturally incompatible with the properties attributed to it.

Two statements from the most credentialed person in the field: backpropagation is not what the brain does, and synchronized parallel copies are not what biological intelligence does. together they say the dominant paradigm is not converging on general intelligence by the mechanism its proponents claim. the field's response has been to largely ignore these statements while continuing to publish scaling law papers that implicitly assume the mechanism is sound.
...
Pre-2012 research had scientific frameworks. PAC learning gave sample complexity bounds growing as O(d/epsilon) for VC dimension d. VC theory bounded generalisation error by model complexity. These were falsifiable, testable, and predictive. Post-2012 deep learning has empirical observations: bigger models perform better usually, more data helps usually, overparameterisation does not hurt somehow. Descriptive, not explanatory. NeurIPS 2012 had 30 percent of papers including theoretical analysis. NeurIPS 2023 had less than 10 percent. Rich Sutton's "bitter lesson" from 2019 stated that general methods leveraging computation are ultimately most effective, which translates to: theory does not matter, just scale. This is engineering philosophy, not science. No theory means no predictions. No predictions means no science. Just empirical tuning. This is what Rahimi called alchemy.
...
## The safety organization that wasn't
...
In 2016, six organizations lobbied in Washington on artificial intelligence. by 2024, that number was 648. the field did not grow 7,567%. the lobbying grew at that rate because the companies recognized that the regulatory environment was the primary threat to their ability to continue deploying inadequately understood systems at scale.
