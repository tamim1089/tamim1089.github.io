...
What’s wild is the attacker chain from initial injection through downloader to full bot. A command injection within the tiny `<language>` XML payload blows past authentication, then spins off external ELF stagers. The downloader’s architecture sniffed ARM or MIPS based on CPU info pulled from `/proc/cpuinfo`, then fetches a matching payload, revealed by directory listings from the command and control server like `/arm5` or `/h/` directories packed with historical binaries. That modular approach means one exploit can hit multiple device architectures seamlessly. This is not hypothetical. FortiGuard Labs observed real exploit traffic in the wild shortly after credential zero‑days were patched. Invalid `<language>` attempts probing device status, dumping config, or dropping downloader payloads. In some cases devices even rewrote their internal reboot commands so administrators couldn’t restore normal operation. In short, attackers weaponized the bug for autonomous Moobot propagation, spreading via mass scanning and automated ELF drop.
